...
def finle_eval(n=sys.maxsize):
    v = n
    num_quarters = len(database.get_quarters())
    if n > num_quarters:
        v = num_quarters
    whights = recommender.generate_weights(v)

    x = num_quarters-v
    out = dict.fromkeys(database.get_all_unique_holdings(), 0)
    for w in whights:
...
def amountinvest(to, frm):  # spesifiy between wich quorter you want the results range  ex if you want (5,5) it will gve you five if you want (5,6) it will give avrege between 5 and 6
    t = time.perf_counter()
    x = (1167483, 1647251, 1009207, 1061768, 1656456, 1273087, 1423053,
         1581811, 909661, 1061165, 1103804, 1350694,  1791786, 1040273)
    out = dict()
    cursor4 = connection.cursor()
    cursor4.execute(
        "SELECT DISTINCT(quarter) FROM raw_13f_data WHERE  put_call is NULL ORDER BY quarter ASC")
    quartes = cursor4.fetchall()
    t = time.perf_counter()
    cursor2 = connection.cursor()
    cursor2.execute("SELECT cusip, value, cik FROM raw_13f_data WHERE quarter=" +
                    "'"+quartes[to][0]+"'"+" AND shareprn_type = 'SH' AND put_call is NULL AND NOT excluded")
    q2 = cursor2.fetchall()
    for fund in x:
        tempdict = dict()
        totalval = 0
